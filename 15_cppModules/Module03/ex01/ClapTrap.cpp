...
ClapTrap::ClapTrap( void )
	: name ("unnamed"), hp (10), ep(10), dp(0) {
		std::cout << "\033[1;36mClapTrap default constructor\033[0m" << std::endl;
}
...
ClapTrap::ClapTrap(const ClapTrap &src) {
	std::cout << "\033[1;36mClapTrap copy constructor called\033[0m" << std::endl;
	*this = src;
}

ClapTrap &ClapTrap::operator=(const ClapTrap &src) {
	std::cout << "\033[1;36mClapTrap copy assignment operator\033[0m" << std::endl;
	if (this != &src) {
		this->name = src.name;
		this->hp = src.hp;
		this->ep = src.ep;
