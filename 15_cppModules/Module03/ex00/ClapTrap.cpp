...
ClapTrap::ClapTrap( std::string name )
	: name (name), hp (10), ep(10), dp(0) {
		std::cout << "\033[1;36mClapTrap " << this->name << " constructed\033[0m" << std::endl;
}

ClapTrap::~ClapTrap( void ) {
	std::cout << "\033[1;36mClapTrap " << this->name << " deconstructed\033[0m" << std::endl;
}

ClapTrap::ClapTrap(const ClapTrap &src) {
	std::cout << "\033[1;36mClapTrap copy " << this->name << " from ClapTrap " << src.name << "\033[0m" << std::endl;
	*this = src;
}

ClapTrap &ClapTrap::operator=(const ClapTrap &src) {
	std::cout << "\033[1;36mCopy assignment called\033[0m" << std::endl;
	if (this != &src) {
		this->name = src.name;
		this->hp = src.hp;
		this->ep = src.ep;
